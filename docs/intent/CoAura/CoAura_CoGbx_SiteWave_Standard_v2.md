# CoAura & SiteWave Standard – Human-facing sites + CoGbx auras v2

> Legacy note: this document was derived from an earlier "CoPortal" standard.
> New work should use CoAura + SiteWave + CoGbx language. The old file is kept
> only for git history and archaeology.
> Purpose: give every CoSuite-aligned site a repeatable, repo-first pattern for  
> building a human front door and a paired, AI-facing CoGbx aura – including  
...
- Future human-facing sites that want a paired CoGbx / Coaura presence.
...
- `docs/intent/CoPortal/` (in CoIndex)
  - this standard and future Coaura patterns

CoIndex remains the **cross-product registry**, with CoBus, Coaura and
...
### Phase 3 – CoGbx / Coaura AI-facing layer
...
- registered in **CoIndex** via:
  - Coaura docs in `docs/intent/CoPortal/`
...
- CoBus / Coaura declarations in CoIndex
...
## 4. CoIndex / CoBus / Coaura registration

For any new site:

1. Create or update a **CoBus schema** (business model) in CoIndex:
   - e.g. `CoBus_{Site}_PayItForward_Schema_v1.md`
2. Create or update a **Coaura registration**:
   - e.g. this doc and any site-specific Coaura notes
